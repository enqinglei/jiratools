...
import urllib
import webbrowser
from rcatrackingconfig import addr_dict,addr_dict1,getjira,teams,get_login_info
...
def api_upload():
    ...
    if f and allowed_file(f.filename):
        ...
        if val == 0:
            ...
            todo = JiraTodo.query.order_by(JiraTodo.PRID.desc()).first()
...
def index():
    ...
    print loginMode
    if loginMode:
        return redirect(url_for('jirahome'))
    else:
        return redirect(url_for('rca_home'))
    ...
    if request.method=='GET':
        if username in admin:
            ...
            return render_template('index.html',count= count,headermessage = headermessage,
                                   ...
                                   user=user)
        elif username in teams and username !='hujun':
            count=Todo.query.filter_by(Handler = username).order_by(Todo.PRClosedDate.asc()).count()
            headermessage = 'All RCA Items'
            return render_template('index.html',count=count,headermessage = headermessage,
                               todos=Todo.query.filter_by(Handler = username).order_by(Todo.PRClosedDate.asc()).all(),\
                               user=user)
        else:
            hello = User.query.filter_by(username=username).first()
            if hello:
                normaluser = hello.email
                count = Todo.query.filter_by(TeamAssessor=normaluser).order_by(Todo.PRClosedDate.asc()).count()
                todos = Todo.query.filter_by(TeamAssessor=normaluser).order_by(Todo.PRClosedDate.asc()).all()
                headermessage = 'All RCA Items'
...
def customer_home():
    ...
    if request.method=='GET':
        if username in admin:
            count = Todo.query.filter_by(JiraRcaBeReqested = 'Yes').order_by(Todo.PRClosedDate.asc()).count()
            todos = Todo.query.filter_by(JiraRcaBeReqested = 'Yes').order_by(Todo.PRClosedDate.asc()).all()
            ...
            count = Todo.query.filter_by(Handler = username,JiraRcaBeReqested = 'Yes').order_by(Todo.PRClosedDate.asc()).count()
            todos = Todo.query.filter_by(Handler = username,JiraRcaBeReqested = 'Yes').order_by(Todo.PRClosedDate.asc()).all()
            headermessage = 'All Cusomer RCA/EDA Items'
            return render_template('customer_index.html',count=count,headermessage = headermessage,
                               todos=todos,user= user)
        else:
    ...
    if request.method=='GET':
        if g.user.username in admin:
            ...
            return render_template('longcycletimerca_index.html',
                                   ...
                                   user=User.query.get(g.user.id).username)
...
def newap():
    if request.method == 'POST':
        if not request.form['APID']:
            ...
            # New added for JIRA
            InjectionRootCauseEdaCause = ''  # RCA/EDA RootCause
            RcaEdaCauseType = ''
            RcaEdaActionType = ''
            TargetRelease = ""
            CustomerAp = "No"
            ApCategory = 'RCA'  # RCA/EDA
            ShouldHaveBeenDetected = ''
            ApJiraId = ""
            # End of added new field for JIRA
    ...
    if user is None or user.check_password(password) is False:
        ...
        if not user:
            ...
            if r.ok:
                ...
                squadGroupName = ''
            user = JiraUser(username, email, displayName, lineManagerAccountId, lineManagerDisplayName,
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            if r.ok:
                ...
                squadGroupName = ''
            user = JiraUser(username, email, displayName, lineManagerAccountId, lineManagerDisplayName, lineManagerEmail,
                        squadGroupName)
            db.session.add(user)
            db.session.commit()
...
def loadProntoInfo():
    ...
    if request.method == 'POST':
        PRID = request.form['PRID'].strip()
        todo = JiraTodo.query.filter_by(PRID=PRID).first()
...
def getLineManagerEmailName(JiraIssueAssignee):
    ...
    user = JiraUser.query.filter_by(email=AssignTo).first()
    if user:
        AssignTo = user.lineManagerEmail
    else:
        try:
            conn = get_ldap_connection()
        except:
...
def create5WhyRcaTask(jira,PRID,request,JiraIssueAssignee):
    ...
    if todo_item:  #
        ...
        todo_item.JiraIssueAssignee = request.form['AssignTo'].strip()
        ...
        JiraIssueAssignee = request.form['AssignTo'].strip()
        todo = JiraTodo(JiraIssueId, JiraIssuePriority, JiraIssueSourceList, JiraIssueBusinessUnit, IsLongCycleTime, \
                    JiraIssueBusinessLine, JiraIssueProductLine, JiraIssueCustomerName, JiraIssueFeature,
                    JiraIssueFeatureComponent, \
                    JiraIssueOther, JiraIssueType, JiraIssueCaseType, JiraIssueStatus, JiraIssueLabels,
                    JiraIssueAssignee, \
                    JiraIssueReporter, PRID, PRTitle, PRRelease, PRSeverity, PRGroupInCharge, PRAttached,
                    PRRcaEdaAssessor, \
                    PRProduct, ReportedBy, FaultCoordinator, JiraIssueSummary, CustomerName, APDueDate)
        todo.user = g.user
        db.session.add(todo)
...
def addToJiraTodo(issue,PRID):
    ...
    if todo_item:  # PR in the rca table,just do the assignment and update assignee.
        ...
        if ReportedBy == 'Customer':
            CustomerName = a['customerName']
        APDueDate = ''
        if JiraIssueType in ('Action for RCA', 'Action for EDA'):
            APDueDate = issue.fields.customfield_37070
        todo = JiraTodo(JiraIssueId, JiraIssuePriority, JiraIssueSourceList, JiraIssueBusinessUnit, \
        ...
        db.session.add(todo)
        db.session.commit()
        flash('JiraTodo item was successfully created')
...
def createAnalysisSubtask(jira,PRID,CaseType,JiraIssueAssignee):
    ...
    existFlag = False
    for issue in issues:
        existFlag = True # Parent issue exist.
    ...
    if existFlag:
        ...
        if not subtasks: # No subtask, just create as it request
            ...
            for task in subtasks:
                if task.fields.issuetype == 'Analysis subtask':
                    if task.fields.customfield_10464 == CaseType:
                        duplicateFlag = True
                        #return None, ErrorCode[1][CaseType] # If exist return directly.
            if duplicateFlag: # If exist return directly.
                return None, ErrorCode[1][CaseType]
            else: # No duplicate, just create
                RcaAnalysisSubtaskKey = issue.key
                summaryinfo = '%s for '%CaseType + PRID
                CaseType = AnalysisIssueCaseType[CaseType]
                issueaddforRCAsubtask = {
...
@app.route('/jiranew', methods=['GET', 'POST'])
@login_required
def jiranew():
    global gSSOPWD
    if request.method == 'POST':
    ...
    if request.method == 'POST':
        ...
        if value == 'RegisterNewGroupInChargeName':
            if not request.form['InChargeGroupName']:
                flash('InChargeGroupName is required', 'error')
...
@login_required
def importinchargegroupfromexcel():
    flash('No support yet')
    return redirect(url_for('InChargeGroupIndex'))
    ...
    if request.method=='GET':
        ...
        for issue in issues:
            ...
            if todo_item:  # PR in the rca table,just do the assignment and update assignee.
                ...
                db.session.add(todo)
                db.session.commit()
                flash('RCA/EDA item was successfully loaded')
        hello = JiraUser.query.filter_by(username=g.user.username).first()
        if hello:
            normaluser = hello.displayName
            JiraIssueAssignee = normaluser
            count = JiraTodo.query.filter_by(JiraIssueAssignee=JiraIssueAssignee).count()
            todos = JiraTodo.query.filter_by(JiraIssueAssignee=JiraIssueAssignee).all()
            headermessage = 'All MNRCA JIRA RCA/EDA Items Assigned To Me'
            return render_template('jiraindex.html', count=count, headermessage=headermessage,
                                   todos=todos, user=JiraUser.query.get(g.user.id).displayName)
...
def index_by_reporter():
    if request.method=='GET':
        ...
        if hello:
            normaluser = hello.displayName
            JiraIssueAssignee = normaluser
            count = JiraTodo.query.filter_by(JiraIssueReporter=JiraIssueReporter).count()
            todos = JiraTodo.query.filter_by(JiraIssueReporter=JiraIssueReporter).all()
            headermessage = 'All MNRCA JIRA RCA/EDA Reported By Me'
            return render_template('jiraindex.html', count=count, headermessage=headermessage,
                                   todos=todos, user=JiraUser.query.get(g.user.id).displayName)
...
def jirashow_or_update(JiraIssueId):
    ...
    if request.method == 'GET':
        if todo_item.JiraIssueType in ('Analysis subtask'):
            return render_template('create_ap_view.html',todo=todo_item)
        elif todo_item.JiraIssueType == '5WhyRCA':
            return render_template('create_analysis_subtask_view.html', todo=todo_item)
        ...
        if value == 'CreatAnalysisSubtask':
            ...
            RcaSubtaskIssue = jira.issue(RcaSubtaskJiraKey)
            Rca5WhyParentIssue = RcaSubtaskIssue.fields.parent
            ...
            # if len(todos) != 0:
            for todo_item in todos:
                ChildSubtaskType = JiraChildIssueType[todo_item.ApCategory] # Excel table RCA/EDA type
                if todo_item.ApCategory == 'RCA':
                    TitleSummary = 'RCA Action For ' + PRID
                else:
                    TitleSummary = 'EDA Action For ' + PRID
                ...
                if todo_item.ApCategory == 'RCA':
                    ...
                    #newissue = jira.create_issue(issueaddforrca_action)
                    if todo_item.ApJiraId == '': # No JIRA Action Issue Created yet,then Create it!
                        newissue = jira.create_issue(issueaddforrca_action)
                    else: # Exist then just update the Jira Issue.
                        JiraIssueId = todo_item.ApJiraId
                        print JiraIssueId
                        newissue = jira.issue(JiraIssueId)
                        newissue.update(issueaddforrca_action)
                ...
                todo_item.ApJiraId = JiraIssueId
                todo_item.APAssingnedTo = JiraIssueAssignee
                db.session.commit()   # Update JiraAPStatus table for ApJiraId and Assignee
            ...
            if shouldhavebeenfound:  # Should have been found update
                ...
                shouldhavebeenfound = []
            status = RcaSubtaskIssue.fields.status
            print status
            if status != 'Closed':
                jira.transition_issue(RcaSubtaskIssue, JIRA_STATUS['Resolved'])
            status = str (RcaSubtaskIssue.fields.status)
            todo_item.PRRcaCompleteDate = time.strftime('%Y-%m-%d', time.localtime(time.time()))
            todo_item.JiraIssueStatus = status
            todo_item.IsRcaCompleted = 'Yes'
            db.session.commit()
...
def jirarca_home():
	...
	if hello:
		normaluser = hello.email
		count = JiraTodo.query.filter_by(user_id=g.user.id).count()
		todos = JiraTodo.query.filter_by(user_id=g.user.id).all()
		headermessage = 'All MNRCA Items'
		return render_template('jiraindex.html', count=count, headermessage=headermessage,todos= todos,\
							   user=JiraUser.query.get(g.user.id).displayName)
...
@login_required
def syslog_view():
    contents = SystemLog.query.order_by(SystemLog.logTime.desc()).all()
